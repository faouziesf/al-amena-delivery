...
<div class="space-y-6">
    ...
    <div class="flex items-center space-x-4">
        <a href="{{ route('depot-manager.commercial.topup-requests.index') }}"
            ...
            @if($topupRequest->status === 'PENDING')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-orange-200 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Approuver -->
                        <form method="POST" action="{{ route('depot-manager.commercial.topup-requests.approve', $topupRequest) }}" class="space-y-4">
                            @csrf
                            <div>
                        ...
                        <!-- Rejeter -->
                        <form method="POST" action="{{ route('depot-manager.commercial.topup-requests.reject', $topupRequest) }}" class="space-y-4">
                            @csrf
                            <div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-orange-200 p-6">
                ...
                <div class="flex items-center space-x-4 mb-4">
                    <div class="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center">
                        <span class="text-orange-800 font-bold">{{ strtoupper(substr($topupRequest->client->name, 0, 2)) }}</span>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-900">{{ $topupRequest->client->name }}</p>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm text-gray-900">{{ $topupRequest->client->phone ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm text-gray-500">Statut</span>
                        <span class="text-sm text-gray-900">{{ $topupRequest->client->account_status }}</span>
                    </div>
                    @if($topupRequest->client->userWallet)
                        <div class="flex justify-between">
                            <span class="text-sm text-gray-500">Solde actuel</span>
                            <span class="text-sm font-medium text-orange-600">{{ number_format($topupRequest->client->userWallet->balance, 3) }} DT</span>
                        </div>
                    @endif
                </div>
            </div>

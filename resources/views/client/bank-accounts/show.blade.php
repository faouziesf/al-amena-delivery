...
<div class="max-w-7xl mx-auto sm:py-5 lg:py-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between flex-col sm:flex-row">
            ...
            <div class="flex space-x-3 flex-col sm:flex-row">
                <a href="{{ route('client.bank-accounts.edit', $bankAccount) }}"
                   class="inline-flex items-center px-4 sm:px-5 lg:px-6 py-3 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:bg-blue-700 text-white rounded-2xl transition-all transform hover:scale-105 active:scale-95 transition-all duration-200 flex-col sm:flex-row">
    ...
    @if($bankAccount->is_default)
        <div class="mb-6">
            <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-green-100 text-green-800 transform hover:scale-105 active:scale-95 transition-all duration-200 flex-col sm:flex-row">
                <svg class="w-6 h-6 sm:w-5 sm:h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-2 sm:gap-3 lg:gap-4 lg:gap-6">
        <!-- Informations principales -->
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-5 lg:p-6 transition-all duration-300 hover:-translate-y-1">
                ...
                <div class="space-y-6">
                    ...
                    <div class="border-t pt-6">
                        ...
                        <p class="text-lg font-mono text-gray-900 bg-gray-50 p-3 rounded-2xl">
        ...
        <div class="lg:col-span-1">
                ...
                <div class="space-y-3">
                    @if(!$bankAccount->is_default)
                        <form action="{{ route('client.bank-accounts.set-default', $bankAccount) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full flex items-center justify-center px-4 py-3 bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 hover:bg-green-700 text-white rounded-2xl transition-colors flex-col sm:flex-row">
                                <svg class="w-6 h-6 sm:w-5 sm:h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                </svg>
                                Définir par défaut
                    ...
                    <a href="{{ route('client.bank-accounts.edit', $bankAccount) }}" 
                       class="w-full flex items-center justify-center px-4 py-3 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:bg-blue-700 text-white rounded-2xl transition-colors flex-col sm:flex-row">
                        <svg class="w-6 h-6 sm:w-5 sm:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                        Modifier
                    ...
                    @if($bankAccount->canBeDeleted())
                        <form action="{{ route('client.bank-accounts.destroy', $bankAccount) }}" method="POST"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full flex items-center justify-center px-4 py-3 bg-red-600 hover:bg-red-700 text-white rounded-2xl transition-colors flex-col sm:flex-row">
                                <svg class="w-6 h-6 sm:w-5 sm:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mt-6 bg-blue-50 border border-blue-200 rounded-2xl p-4">
                <div class="flex flex-col sm:flex-row">
                    <svg class="w-6 h-6 sm:w-5 sm:h-5 text-blue-400 mt-0.5" fill="currentColor" viewBox="0 0 20 20">

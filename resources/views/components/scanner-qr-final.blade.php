...
function scannerQRFinal() {
    return {
        ...
        startAlternatingScans() { this.scanInterval = setInterval(() => { this.scanCycle++; if (this.scanCycle % 2 === 0) { this.scanMode = 'barcode'; } else { this.scanMode = 'qr'; this.analyzeQRFrame(); } }, 750); },
        ...
        onCodeDetected(code, type) { const now = Date.now(); if (this.lastDetection && (now - this.lastDetection.time < 2000) && this.lastDetection.code === code) { return; } this.lastDetection = { code, time: now, type }; this.stopCamera(); if (navigator.vibrate) { navigator.vibrate(100); } this.processCode(code); },
        isValidPackageCode(code) { if (!code || code.length < 6) return false; const cleanCode = code.trim().toUpperCase(); if (/^https?:\/\/.*\/track\//.test(cleanCode)) return true; const obviousWords = ['LIVRAISON', 'DELIVERY', 'BON', 'ALAMENA', 'SERVICE', 'CONTACT', 'TELEPHONE', 'ADRESSE', 'CLIENT', 'DATE']; if (obviousWords.some(word => cleanCode.includes(word) && word.length > 4)) return false; return true; },
        isValidCode(code) { return this.isValidPackageCode(code); },
        validateCode() { this.codeValid = this.isValidPackageCode(this.manualCode); },
        searchCode() { if (!this.codeValid || this.searching) return; this.processCode(this.manualCode.trim().toUpperCase()); },

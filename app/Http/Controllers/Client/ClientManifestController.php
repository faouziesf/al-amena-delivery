...
class ClientManifestController extends Controller
{
    ...
    /**
     * Supprimer un manifeste (seulement si éligible)
     */
    public function destroy($manifestId)
    {
        ...
        if (!$manifest->canBeDeleted()) {
            return response()->json([
                'success' => false,
                'message' => 'Ce manifeste ne peut pas être supprimé car au moins un colis a déjà été ramassé.'
            ], 400);
        }
        ...
        try {
            ...
            if ($manifest->pickupRequest) {
                $manifest->pickupRequest->update([
                    'status' => 'CANCELLED',
                    'cancellation_reason' => 'Manifeste supprimé par le client'
                ]);
            }
